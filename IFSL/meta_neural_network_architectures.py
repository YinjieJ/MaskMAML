...
class MetaLinearLayer(nn.Module):
    def __init__(self, input_shape, hidden_dim, use_bias):
        """
        ...
        self.use_bias = use_bias
        self.weights = nn.Parameter(torch.ones(hidden_dim, c))
...
class MetaLinearNormLayerReLU(nn.Module):
    def __init__(self, input_shape, hidden_dim, use_bias, args, normalization=True,
                 meta_layer=True, no_bn_learnable_params=False, device=None):
        """
           Initializes a BatchNorm->Conv->ReLU layer which applies those operation in that order.
           :param args: A named tuple containing the system's hyperparameters.
    ...
    def build_block(self):
        ...
        self.linear = MetaLinearLayer(input_shape=out.shape, hidden_dim=self.hidden_dim, use_bias=self.use_bias)
...
class FCNReLUNormNetwork(nn.Module):
    def __init__(self, input_shape, args, device, meta=True):
        """
    ...
    def build_network(self):
        ...
        for i in range(self.num_stages):
            self.layer_dict['linear{}'.format(i)] = MetaLinearNormLayerReLU(input_shape=out.shape,
        ...
        self.layer_dict['linear'] = MetaLinearLayer(input_shape=out.shape, hidden_dim=1, use_bias=True)
        #self.layer_dict['sample_weight'] = MetaLinearLayer(input_shape=out.shape, hidden_dim=1, use_bias=True)
    ...
    def forward(self, x, num_step, params=None, training=False, backup_running_statistics=False, mixup=None, lam=None):
        ...
        if (mixup is not None) and layer_mix == 0:
            out0 = lam * out[:nx,:] + (1-lam) * out[nx:,:]
            out = torch.cat((out0, out0), dim=0)
            
        for i in range(self.num_stages):
            out = self.layer_dict['linear{}'.format(i)](out, params=param_dict['linear{}'.format(i)], training=training,
                                                        backup_running_statistics=backup_running_statistics,
                                                        num_step=num_step)
            if (mixup is not None) and layer_mix == (i + 1):
                out0 = lam * out[:nx,:] + (1-lam) * out[nx:,:]
                out = torch.cat((out0, out0), dim=0)

        out1 = self.layer_dict['linear'](out, param_dict['linear'])
        #out2 = self.layer_dict['sample_weight'](out, param_dict['sample_weight'])
...
class IFCNReLUNormNetwork(nn.Module):
    def __init__(self, input_shape, args, device, meta=True):
        """
        Builds a multilayer convolutional network. It also provides functionality for passing external parameters to be
        used at inference time. Enables inner loop optimization readily.
        ...
        be enabled.
        """
        super(IFCNReLUNormNetwork, self).__init__()
        self.device = device
        self.total_layers = 0
        self.args = args
        self.upscale_shapes = []
    ...
    def build_network(self):
        ...
        shape[-1] = shape[-1]//self.n
        self.layer_dict['linear'] = MetaLinearLayer(input_shape=shape, hidden_dim=1, use_bias=True)
        #self.layer_dict['sample_weight'] = MetaLinearLayer(input_shape=out.shape, hidden_dim=1, use_bias=True)
        out = self.g(out)[0]
        out = self.layer_dict['linear'](out)
    ...
    def g(self, x, threshold=1e-3):
        out = []
        d = x.shape[-1]//self.n
        for i in range(self.n):
            outi = x[:,d * i:d * (i+1)]
            zero = torch.zeros_like(outi)
            outi = torch.where(outi < threshold, zero, outi)
            out.append(outi)
        return out
    ...
    def forward(self, x, num_step, params=None, training=False, backup_running_statistics=False, mixup=None, lam=None):
        ...
        out = self.g(out)
        out3 = torch.zeros(1).cuda()
        for i in range(self.n):
            out1 = self.layer_dict['linear'](out[i], param_dict['linear'])
            #out2 = self.layer_dict['sample_weight'](out, param_dict['sample_weight'])
...
class RoutedFCNReLUNormNetwork(nn.Module):
    def __init__(self, input_shape, args, device, meta=True):
        """
        Builds a multilayer convolutional network. It also provides functionality for passing external parameters to be
        used at inference time. Enables inner loop optimization readily.
    ...
    def forward(self, x, num_step, params=None, training=False, backup_running_statistics=False, mixup=None, lam=None):
        ...
        if mixup is not None:
            layer_mix = np.random.randint(0, self.num_stages)
